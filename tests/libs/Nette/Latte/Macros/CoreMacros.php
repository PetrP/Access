...
class CoreMacros extends MacroSet
{
	...
	{
		return array('list($_l, $_g) = CoreMacros::initRuntime($template, '
	...
	{
		$code = $writer->write('CoreMacros::includeTemplate(%node.word, %node.array? + $template->getParameters(), $_l->templates[%var])',
	...
	public function macroEndForeach(MacroNode $node, PhpWriter $writer)
	{
		if (preg_match('#\W(\$iterator|include|require|get_defined_vars)\W#', $this->getCompiler()->expandTokens($node->content))) {
			$node->openingCode = '<?php $iterations = 0; foreach ($iterator = $_l->its[] = new SmartCachingIterator('
	...
	{
		return $writer->write('echo Html::el(NULL, %node.array)->attributes()');
	}
	...
	public function macroDump(MacroNode $node, PhpWriter $writer)
	{
		...
		return 'Debugger::barDump(' . ($node->args ? "array(" . $writer->write('%var', $args) . " => $args)" : 'get_defined_vars()')
	 ...
	 * @return \stdClass
	 */
	public static function initRuntime(ITemplate $template, $templateId)
	{
		// local storage
