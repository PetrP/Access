...
class UIMacros extends MacroSet
{
	...
	public function finalize()
	{
		...
		if ($this->namedBlocks || $this->extends) {
			...
			$prolog[] = '$_l->extends = '
				. ($this->extends ? $this->extends : 'empty($template->_extended) && isset($_control) && $_control instanceof Presenter ? $_control->findLayoutTemplateFile() : NULL')
				. '; $template->_extended = $_extended = TRUE;';

			$prolog[] = '
if ($_l->extends) {
	' . ($this->namedBlocks ? 'ob_start();' : 'return CoreMacros::includeTemplate($_l->extends, get_defined_vars(), $template)->render();') . '
...
if (!empty($_control->snippetMode)) {
	...
	public function macroInclude(MacroNode $node, PhpWriter $writer)
	{
		...
		if (isset($this->namedBlocks[$destination]) && !$parent) {
			...
			$cmd = 'UIMacros::callBlock' . ($parent ? 'Parent' : '') . "(\$_l, $name, %node.array? + " . ($parent ? 'get_defined_vars' : '$template->getParameters') . '())';
		}
	...
	{
		return $writer->write('CoreMacros::includeTemplate(%node.word, %node.array? + get_defined_vars(), $_l->templates[%var])->render()',
	...
	public function macroBlock(MacroNode $node, PhpWriter $writer)
	{
		...
		$prolog = $this->namedBlocks ? '' : "if (\$_l->extends) { ob_end_clean(); return CoreMacros::includeTemplate(\$_l->extends, get_defined_vars(), \$template)->render(); }\n";
	...
	{
		return $writer->write(($node->args ? 'try { $_presenter->link(%node.word, %node.array?); } catch (InvalidLinkException $e) {}' : '')
	...
	public function macroContentType(MacroNode $node, PhpWriter $writer)
	{
		if (Strings::contains($node->args, 'xhtml')) {
			$this->getCompiler()->setContentType(LatteCompiler::CONTENT_XHTML);

		} elseif (Strings::contains($node->args, 'html')) {
			$this->getCompiler()->setContentType(LatteCompiler::CONTENT_HTML);

		} elseif (Strings::contains($node->args, 'xml')) {
			$this->getCompiler()->setContentType(LatteCompiler::CONTENT_XML);

		} elseif (Strings::contains($node->args, 'javascript')) {
			$this->getCompiler()->setContentType(LatteCompiler::CONTENT_JS);

		} elseif (Strings::contains($node->args, 'css')) {
			$this->getCompiler()->setContentType(LatteCompiler::CONTENT_CSS);

		} elseif (Strings::contains($node->args, 'calendar')) {
			$this->getCompiler()->setContentType(LatteCompiler::CONTENT_ICAL);

		} else {
			$this->getCompiler()->setContentType(LatteCompiler::CONTENT_TEXT);
		}
