 ...
 * @package Nette\Latte
 */
class LatteCompiler extends Object
{
	 ...
	 * Adds new macro.
	 * @param  string
	 * @return LatteCompiler  provides a fluent interface
	 */
	...
	public function compile(array $tokens)
	{
		...
		try {
			foreach ($tokens as $this->position => $token) {
				if ($token->type === LatteToken::TEXT) {
					$this->output .= $token->text;

				} elseif ($token->type === LatteToken::MACRO_TAG) {
				...
				} elseif ($token->type === LatteToken::HTML_TAG_BEGIN) {
					$this->processHtmlTagBegin($token);

				} elseif ($token->type === LatteToken::HTML_TAG_END) {
					$this->processHtmlTagEnd($token);

				} elseif ($token->type === LatteToken::HTML_ATTRIBUTE) {
					$this->processHtmlAttribute($token);

				} elseif ($token->type === LatteToken::COMMENT) {
	...
	private function processHtmlTagBegin(LatteToken $token)
	{
		if ($token->closing) {
			do {
			...
			$htmlNode->isEmpty = in_array($this->contentType, array(self::CONTENT_HTML, self::CONTENT_XHTML))
				&& isset(Html::$emptyElements[strtolower($token->name)]);
			$htmlNode->offset = strlen($this->output);
			$this->setContext(NULL);
		}
		$this->output .= $token->text;
	...
	private function processHtmlTagEnd(LatteToken $token)
	{
		if ($token->text === '-->') {
			$this->output .= $token->text;
			$this->setContext(NULL);
	...
	private function processHtmlAttribute(LatteToken $token)
	{
		$htmlNode = end($this->htmlNodes);
	...
	private function processComment(LatteToken $token)
	{
