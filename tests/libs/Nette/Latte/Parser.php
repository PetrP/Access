...
class Parser extends Object
{
	...
	private function contextText()
	{
		...
		if (!empty($matches['htmlcomment'])) { // <!--
			$this->addToken(LatteToken::HTML_TAG_BEGIN, $matches[0]);
			$this->setContext(self::CONTEXT_COMMENT);

		} elseif (!empty($matches['tag'])) { // <tag or </tag
			$token = $this->addToken(LatteToken::HTML_TAG_BEGIN, $matches[0]);
	...
	private function contextAttribute()
	{
		...
		if (!empty($matches['quote'])) { // (attribute end) '"
			$this->addToken(LatteToken::TEXT, $matches[0]);
			$this->setContext(self::CONTEXT_TAG);
		}
		return $matches;
	}
	...
	private function match($re)
	{
		if ($matches = Strings::match($this->input, $re, PREG_OFFSET_CAPTURE, $this->offset)) {
			...
			if ($value !== '') {
				$this->addToken(LatteToken::TEXT, $value);
			}
	...
	{
		$this->output[] = $token = new LatteToken;
		$token->type = $type;
		$token->text = $text;
	...
	protected function filter()
	{
		...
		if ($token->type === LatteToken::MACRO_TAG && $token->name === '/syntax') {
			$this->setSyntax($this->defaultSyntax);
			$token->type = LatteToken::COMMENT;

		} elseif ($token->type === LatteToken::MACRO_TAG && $token->name === 'syntax') {
			$this->setSyntax($token->value);
			$token->type = LatteToken::COMMENT;

		} elseif ($token->type === LatteToken::HTML_ATTRIBUTE && $token->name === 'n:syntax') {
			$this->setSyntax($token->value);
			$this->syntaxEndTag = '/' . $this->lastHtmlTag;
			$token->type = LatteToken::COMMENT;

		} elseif ($token->type === LatteToken::HTML_TAG_END && $this->lastHtmlTag === $this->syntaxEndTag) {
			$this->setSyntax($this->defaultSyntax);

		} elseif ($token->type === LatteToken::MACRO_TAG && $token->name === 'contentType') {
