...
class Cache extends Object implements ArrayAccess
{
	...
	{
		$key = func_get_args();
		return $this->load($key, create_function('', 'extract(NCFix::$vars['.NCFix::uses(array('function'=>$function,'key'=> $key)).'], EXTR_REFS);
			array_shift($key);
	 ...
	 * @param  mixed  key
	 * @return CachingHelper|NULL
	 */
	public function start($key)
	{
		$data = $this->load($key);
		if ($data === NULL) {
			return new CachingHelper($this, $key);
		}
		echo $data;
	}
