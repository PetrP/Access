 ...
 *
 * @author     David Grudl
 * @property-read DIServiceDefinition[] $definitions
 * @property-read array $dependencies
 * @package Nette\DI
 */
class DIContainerBuilder extends Object
{
	...
	public function addDefinition($name)
	{
		...
		return $this->definitions[$name] = new DIServiceDefinition;
	}
	...
	public function autowireArguments($class, $method, array $arguments)
	{
		...
		return DIHelpers::autowireArguments($rm, $arguments, $this);
	}
	...
	public function prepareClassList()
	{
		...
		foreach ($this->definitions as $name => $def) {
			...
			if ($def->class) {
				...
				if (!$def->factory) {
					$def->factory = new DIStatement($def->class);
				}
			} elseif (!$def->factory) {
		...
		foreach ($this->definitions as $name => $def) {
			...
			$def->class = ClassReflection::from($def->class)->getName();
			if ($def->autowired) {
		...
		foreach ($this->classes as $class => $foo) {
			$this->addDependency(ClassReflection::from($class)->getFileName());
		}
	}
	 ...
	 * @return DIContainerBuilder  provides a fluent interface
	 */
	...
	private function generateService($name)
	{
		...
		$code = '$service = ' . $this->formatStatement(DIHelpers::expand($def->factory, $parameters, TRUE)) . ";\n";
		...
		foreach ((array) $def->setup as $setup) {
			$setup = DIHelpers::expand($setup, $parameters, TRUE);
	...
	{
		return DIHelpers::expand($value, $this->parameters, TRUE);
	}
	...
	public function normalizeEntity($entity)
	{
		...
		if (is_array($entity) && $entity[0] instanceof DIServiceDefinition) { // [ServiceDefinition, ...] -> [@serviceName, ...]
			$tmp = array_keys($this->definitions, $entity[0], TRUE);
			$entity[0] = "@$tmp[0]";

		} elseif ($entity instanceof DIServiceDefinition) { // ServiceDefinition -> @serviceName
			$tmp = array_keys($this->definitions, $entity, TRUE);
			$entity = "@$tmp[0]";

		} elseif (is_array($entity) && $entity[0] === $this) { // [$this, ...] -> [@container, ...]
			$entity[0] = '@' . DIContainerBuilder::THIS_CONTAINER;
		}
